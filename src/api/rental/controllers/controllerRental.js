...
const store = async (req, res) => {
  try {
    ...
    if (movies) {
      const returnPeriod = await deadLineReturn(movies.amount, rental.amount);
      if (returnPeriod) {
        const contractRental = await returnObjectRental(rental, returnPeriod);
        ...
        if (amount.status !== 400) {
          ...
          if (!error) {
            ...
            return res.status(201).json(storeRental);
          }
          return res.status(400).json({
            error: error.message
          });
...
const devolveRental = async (req, res) => {
  try {
    ...
    if(verifyDare.status === 200){
      ...
      await Rental.update({returnDate: dateDevolve}, {where: { id }});
